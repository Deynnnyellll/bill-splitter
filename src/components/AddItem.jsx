...
const Additem = ({ toggleAddItem, setAddlist }) => {
  ...
  const handleSubmit = (event) => {
    ...
    toggleAddItem({ name, amount, count })
  ...
  return (
    <div className='pt-2 border-b-[1px] border-[#2a2a2c]'>
      <form className='flex justify-between items-center' onSubmit={handleSubmit}>
        <div className='grid grid-cols-2 grid-flow-row gap-2 my-2'>
          <div className='col-span-2 bg-darkOne p-2 text-[14px] rounded-md'>
            <input type='text' placeholder='Item name' value={name} className={`${styleInput} w-full`} onChange={(event) => setName(event.target.value)} required />
          </div>
        ...
        <div className='flex flex-col gap-2 text-[18px]'>
          ...
          <div className='bg-darkOne ms-4 p-1 rounded-lg cursor-pointer' onClick={() => setAddlist(prev => !prev)}>
            <GoX />
          </div>
        </div>
      </form>
